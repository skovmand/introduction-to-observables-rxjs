...
function typewriter(string, time) {
  ...
  return zip(char$, interval$, (char, interval) => char).pipe(
...
headingTypewriter$.subscribe(
  text => (headingNode.textContent = text)
);

paragraphTypewriter$.subscribe(
  text => (paragraphNode.textContent = text)
);
